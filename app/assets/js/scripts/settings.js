    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
                isLastAccount = true
                setOverlayContent(
                    'Это ваш единственный аккаунт',
                    'Для работы лаунчера требуется наличие, как минимум, одного аккаунта. После удаления вы будете перенаправлены на страницу входа. Продолжить?',
                    'Да, войду с другим аккаунтом',
                    'Отмена'
                )
                setOverlayHandler(() => {
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Игрок</div>
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Выбран &#10004;' : '>Выбрать'}</button>
...
function populateVersionInformation(version, valueElement, titleElement, checkElement) {
    ...
    if (isPrerelease(version)) {
        titleElement.innerHTML = 'Пре-релиз'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = 'Стабильная сборка'
